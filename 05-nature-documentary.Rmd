...
```cs
UI.ShowHelpMessage("Hello", 1000, false);
```

Help Messages accept different color modifiers and input keys
```cs
UI.ShowHelpMessage("~r~Red ~y~Yellow ~g~Green ~w~White ~b~Blue ~n~~w~Press ~INPUT_CONTEXT~");
```
...
```cs
if (HelpMessageOff == false)     //Check loading state , & if text as been closed
{
	UI.ShowHelpMessage("~Press ~INPUT_CONTEXT~ to close this message");
};
if (Game.IsControlJustPressed(0, GTA.Control.Context))//If user press E 
{ 
	HelpMessageOff = true; 
}; 
```
