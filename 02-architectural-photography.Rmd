...
On windows there are several ways to take a screenshot. To capture your entire screen and automatically save the screenshot, press the ```Windows logo key``` + ```PrtScn key```, or alternatively ```fn key``` + ```Windows logo key``` + ```PrtScn key```. The screenshot will be saved to the ```Pictures > Screenshots folder```.
