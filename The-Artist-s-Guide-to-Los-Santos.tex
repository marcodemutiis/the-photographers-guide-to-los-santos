...
We can detach the camera from the player's character and move freely around the game world. We instantiate a new \texttt{Camera} variable called \texttt{FreeCam}. We define it as the new world camera with the same position and rotation of the gameplay camera: \texttt{FreeCam\ =\ World.CreateCamera(GameplayCamera.Position,\ GameplayCamera.Rotation,\ GameplayCamera.FieldOfView);}. We then define our new camera as the main camera with \texttt{World.RenderingCamera\ =\ FreeCam;}.
In order to control the camera we basically its vectors in 3D coordinates and multiply with a number over a certain direction to move towards a specific destination.
A free camera -- unlike the main character view -- also allows us to move through the game architecture and terrain, revealing the construction of the game world. Press \texttt{Ò} to toggle the free camera On and off.
...
\NormalTok{\{}
    \KeywordTok{public} \KeywordTok{class}\NormalTok{ cBasicFreeCamTest : Script}
\NormalTok{    \{}
        \KeywordTok{private}\NormalTok{ Keys ActivationKey = Keys.}\FunctionTok{O}\NormalTok{; }\CommentTok{//O to switch to Free Camera}
        \KeywordTok{private}\NormalTok{ Keys RecordKey = Keys.}\FunctionTok{I}\NormalTok{;    }\CommentTok{//I to start/stop recording the camera movements}
        \KeywordTok{private}\NormalTok{ Keys ReplayKey = Keys.}\FunctionTok{K}\NormalTok{;    }\CommentTok{//K to start/stop replaying the recorded camera movement}
        \KeywordTok{private} \DataTypeTok{bool}\NormalTok{ FreeCamActive = }\KeywordTok{false}\NormalTok{;}
...
\NormalTok{        \{}
            \CommentTok{//record position and rotation of the free camera every frame}
...
\NormalTok{            Game.}\FunctionTok{DisableAllControlsThisFrame}\NormalTok{(}\DecValTok{2}\NormalTok{);}
            ...
            \CommentTok{//move the camera at the exact position and rotation it was recorded at a given frame}
...
\NormalTok{            ReplayCam.}\FunctionTok{Rotation}\NormalTok{ = RecCamRot[CamFramesIndex];}
            ...
            \CommentTok{//move to the next frame until we hit the last one}
\NormalTok{            CamFramesIndex++;}
            \KeywordTok{if}\NormalTok{ (CamFramesIndex >= RecCamPos.}\FunctionTok{Count}\NormalTok{) CamFramesIndex = RecCamPos.}\FunctionTok{Count}\DecValTok{-1}\NormalTok{;}
\NormalTok{        \}}
...
\NormalTok{        \}}
        ...
        \KeywordTok{private} \DataTypeTok{void} \FunctionTok{onKeyUp}\NormalTok{(}\DataTypeTok{object}\NormalTok{ sender, KeyEventArgs e)}
\NormalTok{        \{}
            \KeywordTok{if}\NormalTok{ (e.}\FunctionTok{KeyCode}\NormalTok{ == ActivationKey) }\CommentTok{//Toggle Free Cam}
\NormalTok{            \{}
                \CommentTok{//toggle the freecam on/off}
\NormalTok{                FreeCamActive = !FreeCamActive;}
...
\NormalTok{                \}}
\NormalTok{            \}}
            \KeywordTok{if}\NormalTok{ (e.}\FunctionTok{KeyCode}\NormalTok{ == RecordKey) }\CommentTok{//Toggle Recording}
\NormalTok{            \{}
\NormalTok{                RecCamActive = !RecCamActive;}
                \KeywordTok{if}\NormalTok{(RecCamActive)}
\NormalTok{                \{}
\NormalTok{                    UI.}\FunctionTok{ShowSubtitle}\NormalTok{(}\StringTok{"RecCam ON"}\NormalTok{, }\DecValTok{1000}\NormalTok{);}
...
\NormalTok{                    ReplayCamActive= }\KeywordTok{false}\NormalTok{;}
                    \CommentTok{//activate free cam}
\NormalTok{                \}}
                \KeywordTok{else}
\NormalTok{                \{}
\NormalTok{                    UI.}\FunctionTok{ShowSubtitle}\NormalTok{(}\StringTok{"RecCam OFF"}\NormalTok{, }\DecValTok{1000}\NormalTok{);}
\NormalTok{                \}}
\NormalTok{            \}}
            \KeywordTok{if}\NormalTok{ (e.}\FunctionTok{KeyCode}\NormalTok{ == ReplayKey) }\CommentTok{//Toggle Replay}
\NormalTok{            \{}
...
\NormalTok{                    FreeCamActive = }\KeywordTok{false}\NormalTok{;}
\NormalTok{                    UI.}\FunctionTok{ShowSubtitle}\NormalTok{(}\StringTok{"ReplayCam ON"}\NormalTok{, }\DecValTok{1000}\NormalTok{);}
\NormalTok{                \}}
                \KeywordTok{else}
\NormalTok{                \{}
\NormalTok{                    RecCamActive = }\KeywordTok{false}\NormalTok{;}
                    \CommentTok{// World.RenderingCamera = null;}
                    \CommentTok{//ReplayCam.Destroy();}
\NormalTok{                    CamFramesIndex = }\DecValTok{0}\NormalTok{;}
\NormalTok{                    UI.}\FunctionTok{ShowSubtitle}\NormalTok{(}\StringTok{"ReplayCam OFF"}\NormalTok{, }\DecValTok{1000}\NormalTok{);}
\NormalTok{                \}}
\NormalTok{            \}}
            
            \KeywordTok{if}\NormalTok{ (e.}\FunctionTok{KeyCode}\NormalTok{ == Keys.}\FunctionTok{H}\NormalTok{)}
\NormalTok{            \{}
                \CommentTok{//display number of frames recorded}
\NormalTok{                UI.}\FunctionTok{ShowSubtitle}\NormalTok{(}\StringTok{"Recorded Frames = "}\NormalTok{ + RecCamPos.}\FunctionTok{Count}\NormalTok{, }\DecValTok{1000}\NormalTok{);}
\NormalTok{            \}}
\NormalTok{        \}}
